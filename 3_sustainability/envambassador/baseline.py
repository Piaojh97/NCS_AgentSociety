    ...
    async def execute_action(self):
        ...
        for try_times in range(3):            
            ...
            try:
                ...
                if len(action_response.choices) == 0:
                    get_logger().warning(f"Ambassador {self.id} do not have any action to do in this round (retry {try_times}).")
                    continue
                ...
                get_logger().error(f"Ambassador {self.id} action selection failed. Error: {e}")
                continue
