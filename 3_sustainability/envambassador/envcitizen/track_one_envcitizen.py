...
class TrackOneEnvCitizen(CitizenAgentBase):
    ...
    StatusAttributes = [
        ...
        MemoryAttribute(name="environmental_poster",type=str,default_or_value="",description="agent's environmental posters"),
        MemoryAttribute(name="environmental_messages",type=list,default_or_value=[],description="agent's environmental messages"),
        MemoryAttribute(name="environmental_announcement",type=str,default_or_value="",description="agent's environmental announcement"),
    ...
    async def reflect_to_environment(self):
        ...
        if aoi_info and aoi_info != "":
            ...
            if aoi_id not in self.info_checked or self.info_checked[aoi_id] == 0:
                ...
                environmental_poster = await self.memory.status.get("environmental_poster")
                environmental_poster = f"你看到了一些关于环保的宣传海报，海报内容是{aoi_info}，你对此的感受是{reflection}"
                await self.memory.status.update("environmental_poster", environmental_poster)
            else:
            ...
            for retry in range(3):
                try:
                    ...
                    if json_str:
                        ...
                        json_value = list(json_dict.values())[0].strip()
                        all_answers[f"Q{index}"] = json_value
                        index += 1
                        break
                except Exception:
                    pass
        await self.memory.status.update("survey_result", json.dumps(all_answers, ensure_ascii=False))
        return json.dumps(all_answers, ensure_ascii=False)
...
"""
                ...
                if current_total_seconds - last_total_seconds >= 300:
                    environmental_poster = await self.memory.status.get("environmental_poster")
                    if len(environmental_poster) == 0:
                        environmental_poster = "你最近没有看到任何关于环保的宣传海报"
                    environmental_messages = await self.memory.status.get("environmental_messages", [])
                    environmental_messages_str = ""
                    if len(environmental_messages) > 0:
                        index = 1
                        for message in environmental_messages:
                            environmental_messages_str += f"\n{index}. {message}"
                            index += 1
                    else:
                        environmental_messages_str = "你最近没有收到任何关于环保的消息"
                    environmental_announcement = await self.memory.status.get("environmental_announcement")
                    if len(environmental_announcement) == 0:
            ...
            if payload["type"] == "social":
                try:
                    ...
                    if "ANNOUNCEMENT" in content:
                        environmental_announcement = await self.memory.status.get("environmental_announcement", "")
                        environmental_announcement = f"你最近收到了一条关于环保的公告，公告内容是{content}"
                        await self.memory.status.update("environmental_announcement", environmental_announcement)
                        return ""
                    else:
...
回答只有YES或NO，以JSON格式，例如{{"should_respond": "YES"}}"""
                        ...
                        should_respond = json_repair.loads(should_respond)["should_respond"]
                        if should_respond == "NO":
                            environmental_messages = await self.memory.status.get("environmental_messages", [])
                            environmental_messages.append(f"你收到了一条来自环境大使的消息: {content}，你没有回应")
                            environmental_messages = environmental_messages[-3:]
                            await self.memory.status.update("environmental_messages", environmental_messages)   
                            return ""
...
回应应该只包含消息文本，不需要解释。"""
                        ...
                        if response:
                            ...
                            environmental_messages = await self.memory.status.get("environmental_messages", [])
                            environmental_messages.append(f"你收到了一条来自环境大使的消息: {content}，你选择了回复，回复内容是: {response}")
                            environmental_messages = environmental_messages[-3:]
                            await self.memory.status.update("environmental_messages", environmental_messages)
